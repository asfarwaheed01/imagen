...
const TERMINAL_STATUSES = new Set(["edited", "failed", "completed"]);

function allDone(imgs: ImageItem[]): boolean {
  if (!imgs.length) return false;
  return imgs.every((img) => {
    const status = img.job?.status ?? img.status;
...
const OrderDetailPage = () => {
  ...
  const downloadableUrls = images.flatMap((img) => {
    const ext = (url: string) => url.split(".").pop()?.split("?")[0] ?? "jpg";
    const base =
      img.originalFilename?.replace(/\.[^/.]+$/, "") ?? `image-${img.id}`;
    const results: { url: string; filename: string }[] = [];
    const editedUrl = img.editedKey ?? img.job?.resultKey;
    if (editedUrl)
      results.push({
    ...
    <div className="min-h-screen bg-[#f8f9fb] pt-16 pb-10">
      ...
      {downloadableUrls.length > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
          <button
            onClick={async () => {
              for (const { url, filename } of downloadableUrls) {
                try {
                  const blob = await fetch(url).then((r) => r.blob());
                  const objectUrl = URL.createObjectURL(blob);
                  const a = document.createElement("a");

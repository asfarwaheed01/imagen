...
// // ── GCP delete — fire-and-forget, never blocks UI ─────────────────────────────

// function deleteGcpUrl(url: string | undefined): void {
//   if (!url) return;
//   api
//     .delete("/api/upload/temp-image", { data: { url } })
//     .catch(() => console.warn("[UploadForm] GCP delete failed for:", url));
// }
...
//   const anyUploading = form.images.some((m) => m.uploading);
//   const anyFailed = form.images.some((m) => !!m.uploadError);
//   const payEnabled =
//     uploadEnabled &&
...
//         if (data.allDone) {
//           stopPolling();
//           setTimeout(() => router.push(`/library/${orderId}`), 1500);
//         }
//       } catch {}
//     };
//     poll();
...
//         return {
//           ...prev,
//           images: [...prev.images, ...newMeta].slice(
//             0,
//             prev.imageCount || MAX_IMAGES,
//           ),
//         };
//       });
//     },
...
//     setForm((prev) => {
//       deleteGcpUrl(prev.images[i]?.gcpUrl);
...
//     setForm((prev) => {
//       prev.images.forEach((m) => deleteGcpUrl(m.gcpUrl));
//       return { ...prev, images: [] };
//     });
//   }, []);
...
//         gcpUrls: form.images.map((m) => m.gcpUrl ?? ""),
//       });
...
//     if (submitting) return "Preparing payment…";
//     if (anyUploading) return "Uploading images…";
//     if (anyFailed) return "Some uploads failed — retry above";
//     if (form.imageCount > 0) return `Pay $${totalCost} & Send to Editor`;
//     return "Pay & Send to Editor";
//   };
...
//                 const n = Number(v);
//                 setForm((prev) => {
//                   prev.images.slice(n).forEach((m) => deleteGcpUrl(m.gcpUrl));
//                   return {
//                     ...prev,
//                     imageCount: n,
